...
void ImpTypeChecker::visit(DowhileStatement* s) {//3
  buclepila.push_back(s);
  s->body->accept(this);
  ImpType type = s->cond->accept(this);
  if (!type.match(booltype)) {
...
void ImpTypeChecker::visit(LoopSkipStatement* s) {//4
  if (buclepila.size()==0) {
    cout << "LoopSkipStatement fuera de bucle" << endl;
    exit(0);
  }
  return;
}
